...
function PANEL:Think()
	...
	for k, ply in SortedPairs( Wins ) do
		if (counter < 7) then
			...
			line.goodness = true
			line.Locked = ply:GetLocked()
			line.dominating = (ply:GetCombo() >= 3)
		end
		counter = counter + 1
	end
	if (counter > 7) then
...
function PANEL:Paint()
	...
	if LocalPlayer():IsValid() then
		...
		hasLocked = LocalPlayer():GetLocked()
	end
	...
	local hasLockedNot = ( not(hasLocked) and 1 ) or 0
	surface.DrawTexturedRectRotated(self:GetWide()/2, 24, 64 + hasLockedNot*math.cos(CurTime()*16)*4, 64 + hasLockedNot*math.cos(CurTime()*16)*4, 0)
